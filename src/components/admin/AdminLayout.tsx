...
const allSidebarItems = [
  ...
  { id: 'ai' as TabType, label: 'AI Yönetimi', icon: Brain, path: '/admin/ai-management' },
...
const getActiveTabFromPath = (pathname: string, search: string): TabType => {
  ...
  if (pathname === '/admin/ai-management') return 'ai';
...
export const AdminLayout = ({ children, activeTab: propActiveTab }: AdminLayoutProps) => {
  ...
  const sidebarItems = allSidebarItems.filter(item => {
    ...
    // 2FA, yetkilendirme, ai and kullanicilar only visible to super admins
    if (item.id === '2fa' || item.id === 'yetkilendirme' || item.id === 'kullanicilar' || item.id === 'ai') {
      return false;
    }
  ...
  return (
    <div className="min-h-screen bg-background flex">
      {/* Sidebar */}
      <aside
        className={`bg-card border-r border-border flex flex-col shrink-0 transition-all duration-300 ease-in-out ${isCollapsed ? 'w-16' : 'w-64'
          }`}
      >
